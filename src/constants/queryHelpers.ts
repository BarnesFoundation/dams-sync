...
const insertQueryGenerator = (
  table: TableInformation,
  objects: Array<{ [key: string]: any }>
) => {
  if (!objects || objects.length == 0) {
    Logger.debug(
      `No objects provided for insertion to table "${table.tableName}"`,
      objects.join()
    );
  }
  ...
  const onConflictCommand = generateOnConflictCommand(
    table,
    columnNamesToInsert
  );
 ...
 */
const generateOnConflictCommand = (
  table: TableInformation,
  columnNamesToInsert: string[]
): string => {
  ...
  const setList = columnNamesToInsert.reduce((acc, column) => {
    ...
    if (!isColumnPrimary) {
      acc.push(`${column} = EXCLUDED.${column}\n`);
    }
  ...
  const setString =
    setList.length > 0 ? `DO UPDATE SET ${setList.join()}` : "DO NOTHING";
