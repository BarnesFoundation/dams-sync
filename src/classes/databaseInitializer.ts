...
export class DatabaseInitializer {
  ...
  public createMainObjectTable = async () => {
    ...
    const columnsString = mainObjectInformation.columns
  ...
  public createConstituentTable = async () => {
    ...
    const columnsString = constituentRecords.columns
      .map((column) => `"${column.name}" ${column.type}`)
      .join(",\n");
  ...
  public createTextEntryStoreTable = async () => {
    ...
    const columnsString = textEntryStore.columns
      .map((column) => `"${column.name}" ${column.type}`)
      .join(",\n");
  ...
  public createMediaInformationTable = async () => {
    ...
    const columnsString = mediaInformation.columns
      .map((column, index, array) => `"${column.name}" ${column.type}`)
      .join(",\n");
