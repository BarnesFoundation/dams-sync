...
export class SQLConnection {
  ...
  public async connect() {
    try {
      ...
      Logger.debug(
        `Successfully connected to the ${this.type} database and created pool`
      );
    } catch (error) {
      Logger.error(
        `An error occurred connecting to the ${this.type} database`,
        error
      );
    }
  }
  ...
  public async executeQuery(query: string, values?: string[] | number[]) {
    ...
    try {
      ...
      Logger.error(
        `An error occurred running the provided query on the ${this.type} database`,
        query
      );
      Logger.error(error);
    }
